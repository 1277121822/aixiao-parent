	...
	$("#contentCategory").tree({
        ...
        onAfterEdit : function(node){
        	...
        	if(node.id == 0){
        		...
        		//name：新增节点的文本
        		$.post("/user/category/create",{parentId:node.parentId,name:node.text},function(data){
        			if(data.status == 200){
        				//更新节点
        				_tree.tree("update",{
        		...
        		$.post("/user/category/update",{id:node.id,name:node.text});
        	}
        }
	});
});
	...
	if(item.name === "add"){
		...
		$.messager.confirm('确认','确定删除名为 '+node.text+' 的分类吗？',function(r){
			if(r){//如果是true 表示要执行以下的逻辑
				$.post("/user/category/delete",{id:node.id},function(data){
				    if(data.status == 200){
                        //后台删除成功后，删除前端的节点
